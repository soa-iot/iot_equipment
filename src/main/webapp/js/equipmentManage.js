 ...
 */
//新增数据行请求url
var addRowUrl = "insertEquipentData.action",
...
$(function(){	
	...
	var tableColumns = [
			...
			{ field:'WEL_NAME', title:'装置列', align:'center',width:"80" },
			{ field:'WEL_UNIT', title:'装置单元', align:'center',width:"80" },
			{ field:'EQU_MEMO_ONE', title:'设备类别', align:'center',width:"80" },
			{ field:'EQU_POSITION_NUM', title:'设备位号', align:'center',width:"80" },
			{ field:'EQU_NAME', title:'设备名称', align:'center',width:"80" }
	];
	var baseTableColumns = [
			...
			{ field:'WEL_UNIT', title:'装置单元', align:'center',width:"80" },
			{ field:'EQU_MEMO_ONE', title:'设备类别', align:'center',width:"80" },
			{ field:'EQU_POSITION_NUM', title:'设备位号', align:'center',width:"100" },
			{ field:'EQU_NAME', title:'设备名称', align:'center',width:"80" }
	];
	...
	//初始化
	$( '.equipment_highSearch' ).css( {'display': 'none'} );	
...
//				"/" + getUrlParamValueByName ( "currentLastMenoId" ), findAuthoritySuccessFunction );	
	...
	function setCurrentAuthority(){
		...
		if( currentNodeName == "仪表设备" || currentNodeName == "机械设备" 
			...
			$.each( equipementAuthEventConfig, function( key, value ){
				...
				$( equipementAuthorityConfig[key] ).off( 'click', function(){
			...
			$.each( currentUserAuthority, function( index, item ){
				...
				$( equipementAuthorityConfig[item] ).on( 'click', function(){
					eval( eventName + "()" );
				})
				if( equipementAuthorityConfig[item] == "导出" ){
					$( equipementAuthorityConfig[item] ).hover( function(){  
						$("#exportList").show(); 
			    	},function(){  			    	  
			        	$("#exportList").hide();  
			    	})					
				}
				$( equipementAuthorityConfig[item] ).removeClass( 'l-btn-disabled' );
			})
			//设置导出和导出Excel模板
	...
	function highSearchClickFunction(){
		...
		/*if( $highSearchDiv ){			
			if( $highSearchDiv.hasClass( '.equipment_highSearch_itemOpen' ) ){
				$highSearchDiv.removeClass( '.equipment_highSearch_itemOpen' )
						.addClass( '.equipment_highSearch_itemClose' );
			}else{
	...
	$( '.equpment-type' ).tree({		
		onClick: function( node ){
			...
			currentNodeParentName = $( '.equpment-type' ).tree( 'getParent', node.target ).text;
			//console.log( $( node.target ).closest( 'ul' ).before( 'div' ).find( '.tree-title' )  );
			//currentNodeParentName = $( node.target ).closest( 'ul' ).before( 'div' ).find( '.tree-title' ).text();
			...
			if( currentNodeName == "仪表设备" || currentNodeName == "机械设备" 
				...
				$( '.equipment_highSearch' ).css( {'display': 'inline-block'} );
			}
			/*
			 * 动态生成表格展示列
			 */	
			...
			if( currentNodeName == "仪表设备" || currentNodeName == "机械设备" 
				...
				//});
				tableOption.queryParams = {"SECONDCLASS_EQUIPMENT" : currentNodeName};
			}else {
				//$( tableId ).datagrid( 'load', {
		...
		$.each( equipmentStructure, function( key, value ){
			if( currentNodeName == key && currentNodeName != '全部设备'  
				...
				$.each( value, function( key1, value1 ){
					...
					activeHtml = activeHtml + ' <span>' + key1 + ':</span> ' +
	...
	function activeGenerateAddRowHtml(){
			...
			if( currentNodeName == key || "全部设备" == key ){
				$.each( value, function( key1, value1 ){
					middleActiveHtml = middleActiveHtml +
						...
						' 	<label for="name">' + key1 + ':</label>     '  +
						'   	<input class="easyui-validatebox" type="text"    '  +
						'   		name="' + value1 + '" data-options="required:true" /> '  +
						' </div>  ' +
						'<p style="height:5px;margin:0px;padding:0px;"></p>';
				})
			}
		...
		var endHtml = ' <div style="text-align:center;padding:5px">    ' +		
					  ...
					  '     	onclick="addHtmlCancelClick()"	></a> ' +	
					  '	</div>'+ 
					  ' </form> '; 					  
		//console.log( '新增表格列-动态生成……html:' + beforeHtml + middleActiveHtml + endHtml );
	...
	function activeModifyRowHtml(){
		...
		$.each( equipmentStructure, function( key, value ){
			if( currentNodeName == key || "全部设备" == key ){				
				$.each( value, function( key1, value1 ){
					readOnlyInput = "";
					if( value1 == "EQU_MEMO_ONE" || value1 == "EQU_POSITION_NUM" ){
						readOnlyInput = "readonly = readonly" ;
					}
					middleActiveHtml = middleActiveHtml +
						' <div  style="margin-left:40px;"> ' +
						' 	<label for="name">' + key1 + ':</label>     '  +
						'   <input class="easyui-validatebox" type="text" ' + readOnlyInput +
						'   		name="' + value1 + '" data-options=""/> '+
						' </div>  ' +
						'<p style="height:5px;margin:0px;padding:0px;"></p>';
				})
			}
		...
		var endHtml = ' <div style="text-align:center;padding:5px">    ' +		
					  ...
					  '     	onclick="modifyHtmlCancelClick()"	></a> ' +	
					  '	</div>'+ 
					  ' </form> '; 	
		//console.log( '新增表格列-动态生成……html:' + beforeHtml + middleActiveHtml + endHtml );
	...
	function importExcelUploadFunction(){
        ...
        if( file_typename == '.xlsx' || file_typename == '.xls' ){
          ...
          if(file.size > 1024*1024){
            ...
            $( '#fileName' ).append(
            		"<span style='color:Blue'>文件名: " + 
            		file.name + ',大小: ' + fileSize + "</span>"
            )
      	...
      	if( jsonData.state==0 ){
                ...
                error : function ( returnInfo ) {
                    //上传失败时显示上传失败信息
                	$( '#uploadInfo' ).append(
                			 "<span style='color:Red'>" + returnInfo + "</span>"
                	)
                }
            })
        }else {
            ...
            //将错误信息显示在前端label文本中
            $( '#fileName' ).append(
            		"<span style='color:Red'>错误提示:上传文件应该是.xlsx后缀而不应该是" + 
	...
	$( '#menu' ).click( function(){
		...
		if( item.name == "detail" ){
			...
			self.parent.addTab(EQU_POSITION_NUM, url, "icon-add");
	...
	function getHisOperateCF(){
		console.log( '右键行查看历史数据-单击事件……' );
...
function exportXTCF(){
	...
	//定位生成
	$( '#exportList' ).css({'position':"absolute",'top':heights,'left':widths,'z-index':20,
